 ...
 * @Description:
 */
public class BasedModelWavePlane {
...
//            System.out.println("lightpath:"+lightpath.getName()+"  moduFormat:"+lightpath.getModuFormat().getName()+"  channels:"+lightpath.getChannels());
    ...
    public Gv trafficGrooming(LinkedHashMap<String,Nodepair> nodepairlist,LinkedList<Integer>serviceList){
        ...
        satisfyServices(nodepairlist, layer,serviceList1,serviceList2, gv, wavePlaneList, existNodeList);
        gv.setLinklist(layer.getLinklist());
        int maxUsedWaveIndex=0;
        double variance=0;
        double usedSum=0;
        for (Link link:layer.getLinklist().values()) {
            usedSum+=link.getMaxUsedWaveIndex(link);
        ...
        for (Link l:layer.getLinklist().values()) {
            variance+=(l.getMaxUsedWaveIndex(l)-usedSum) * (l.getMaxUsedWaveIndex(l)-usedSum);
        }
        variance=variance / 9 * 0.001;
        double compare=usedSum+variance;
        ...
        for (int shuffleNum = 0; shuffleNum < Constant.shuffle; shuffleNum++) {
            ...
            satisfyServices(nodepairlist1, layer1,serviceList1,serviceList2, newGv, wavePlaneList1, existNodeList);
            newGv.setLinklist(layer1.getLinklist());
            int maxUsedWaveIndex1=0;
            double variance1=0;
            double usedSum1=0;
            for (Link l:layer1.getLinklist().values()) {
                usedSum1+=l.getMaxUsedWaveIndex(l);
                if (maxUsedWaveIndex1<l.getMaxUsedWaveIndex(l))
                    maxUsedWaveIndex1=l.getMaxUsedWaveIndex(l);
            }
            usedSum1=usedSum1/9;
            for (Link l:layer1.getLinklist().values()) {
                variance1+=(l.getMaxUsedWaveIndex(l)-usedSum1) * (l.getMaxUsedWaveIndex(l)-usedSum1);
            }
            variance1=variance1 / 9 * 0.001;
            double compare1=usedSum1+variance1;
            if (compare>compare1){
                compare=compare1;
                gv=newGv;
                layer=layer1;
