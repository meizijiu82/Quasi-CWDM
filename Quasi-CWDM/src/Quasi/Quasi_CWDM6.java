...
//            System.out.println("lightpath:"+lightpath.getName()+"  moduFormat:"+lightpath.getModuFormat().getName()+"  channels:"+lightpath.getChannels());
        ...
        System.out.println("SR  业务大小"+sum+"G"+"   seed:"+Constant.seed+"   业务数"+Constant.serviceSum);
        ...
        for (Nodepair nodepair:nodepairlist.values()) {
            ...
            if (link!=null){
                ...
                System.out.println("remCapacity:"+link.getRemCapacity()+"  channel:"+link.getChannels()+"  usedIndex："+link.getMaxUsedWaveIndex(link));
            }
        }
        ...
        for (Nodepair nodepair:nodepairlist.values()) {
            ...
            Link link= layer.findLink(srcNode,desNode);
            if (link!=null){
                System.out.println(link.getMaxUsedWaveIndex(link));
            }
        }
    ...
    public Gv trafficGrooming(LinkedHashMap<String,Nodepair> nodepairlist,LinkedList<Integer>serviceList){
        ...
        //得到初始拓扑
        satisfyServices(nodepairlist, layer, serviceList, gv, BPSK, QPSK, eightQAM, existNodeList);
        gv.setLinklist(layer.getLinklist());
        int maxUsedWaveIndex=0;
        for (Link link:layer.getLinklist().values()) {
            if (maxUsedWaveIndex<link.getMaxUsedWaveIndex(link))     //波长的最大使用index
                maxUsedWaveIndex= link.getMaxUsedWaveIndex(link);
...
//                maxWave=link.getChannels();
        ...
        for (int shuffleNum = 0; shuffleNum < Constant.shuffle; shuffleNum++) {
            ...
            satisfyServices(nodepairlist1, layer1, serviceList, newGv, BPSK, QPSK, eightQAM, existNodeList);
            newGv.setLinklist(layer1.getLinklist());
            int maxUsedWaveIndex1=0;
            for (Link l:layer1.getLinklist().values()) {
                if (maxUsedWaveIndex1<l.getMaxUsedWaveIndex(l))
                    maxUsedWaveIndex1=l.getMaxUsedWaveIndex(l);
//                if (maxUsedWaveIndex1<l.getChannels())
//                    maxUsedWaveIndex1=l.getChannels();
            }
            if (maxUsedWaveIndex>maxUsedWaveIndex1){
                maxUsedWaveIndex=maxUsedWaveIndex1;
                gv=newGv;
                layer=layer1;
            }
        }
...
//                                        }
                        ...
                        buildShortestLightpath(layer, gv, BPSK, QPSK, eightQAM, existNodeList, service, nodepair, srcNode, desNode, lightpath, vlink, dijkstra, judge);

                        //概率整形PS
//                        buildShortestLightpath(layer, gv,PS, existNodeList, service, nodepair, srcNode, desNode, lightpath, vlink, dijkstra, judge);
                        break Loop;
                    }
                }
            }
    ...
    private void buildShortestLightpath(Layer layer, Gv gv, HashMap<Integer, Double> PS, HashSet<Node> existNodeList, int service,
                                        Nodepair nodepair, Node srcNode, Node desNode, Lightpath lightpath, Vlink vlink,
                                        Dijkstra dijkstra, int judge) {
        ...
        if (judge==0){
            ...
            int distance=desNode.getLength_from_src();
            ...
            gv.setRemCapacity(capacity-nodepair.getDemand()+ gv.getRemCapacity());
            ...
            while (currentNode1!=srcNode){
                ...
                link.setRemCapacity(link.getRemCapacity()+capacity-nodepair.getDemand());
    ...
    static void buildShortestLightpath(Layer layer, Gv gv, ModuFormat BPSK, ModuFormat QPSK, ModuFormat eightQAM, HashSet<Node> existNodeList,
                                       int service, Nodepair nodepair, Node srcNode, Node desNode,
                                       Lightpath lightpath, Vlink vlink, Dijkstra dijkstra, int judge) {
            ...
            if (desNode.getLength_from_src()>=2000){
                for (int i=1;i<10;i++){
                    if (nodepair.getDemand()<BPSK.getCapacity()*i){
                        ...
                        srcNode.setNipB(i+srcNode.getNipB());
                        desNode.setNipB(i+desNode.getNipB());
                        Node currentNode1=desNode;
                        while (currentNode1!=srcNode){
                            Link link=layer.findLink(currentNode1,currentNode1.getParentNode());    //找到光路途经链路
                        ...
                        if (desNode.getLength_from_src()>=4000) lightpath.setNregenB(i);
                        break;
                    }
                }
            }
                ...
                for (int i=1;i<5;i++){
                    if (nodepair.getDemand()<QPSK.getCapacity()*i){
                        ...
                        srcNode.setNipQ(i+srcNode.getNipQ());
                        desNode.setNipQ(i+desNode.getNipQ());
                        Node currentNode1=desNode;
                        while (currentNode1!=srcNode){
...
//                                    }
//                                    else
                    if (nodepair.getDemand()<eightQAM.getCapacity()*i){
                        ...
                        srcNode.setNipM(i+srcNode.getNipM());
                        desNode.setNipM(i+desNode.getNipM());
                        Node currentNode1=desNode;
                        while (currentNode1!=srcNode){
                            Link link=layer.findLink(currentNode1,currentNode1.getParentNode());
                            lightpath.getLplink().add(link);
            ...
            vlink.setCapacity(vlink.getCapacity()+lightpath.getCapacity());             //更新虚拟链路的容量
//                            vlink.setRemCapacity(vlink.getRemCapacity()+lightpath.getRemCapacity());    //更新虚拟链路的剩余容量
...
//                                   System.out.print("途经链路不满足节点对需求！  ");
                                    ...
                                    LinkedList<Integer>routeArr=new LinkedList<>();
                                    for (Route route:routeArrayList) {
                                        routeArr.add(route.getLength());   //得到所有路由的剩余容量
                                    }
...
//                                    routeArr.sort(Collections.reverseOrder());  //按从大到小排序
                                    ...
                                    int routeLength=0;
                                    if (routeArr.size()!=0){
                                        routeLength=routeArr.getFirst();    //排序后的第一个路由
                                    }
                                    for (Route route:routeArrayList){
...
////                                                    lpArr.add(lp.getCapacity());
//                                                    lp.setGrooming(0);
//                                                }
//                                                Collections.sort(lpArr);                    //将光路按剩余容量从小到大排序
