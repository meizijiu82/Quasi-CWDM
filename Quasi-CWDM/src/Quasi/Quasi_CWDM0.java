...
//        Random random=new Random(Constant.serviceSeed);
//        int serviceSum=0;
//        while (serviceSum<=Constant.serviceSum){
//            int num= random.nextInt(92);
//            for (Nodepair nodepair:nodepairlist.values()) {
//                if (nodepair.getIndex()==num&&nodepair.getServiceList().size()==0){
//                    for (int j = 0; j < nodepair.getServiceSum(); j++) {
//                        int serviceDemand=Constant.lowBound+random.nextInt(Constant.bound);
//                        nodepair.getServiceList().put(j,serviceDemand);     //创建节点对之间的业务集合
//                        serviceSum++;
//                    }
//                    break;
//                }
...
//            System.out.println("lightpath:"+lightpath.getName()+"  moduFormat:"+lightpath.getModuFormat().getName()+"  channels:"+lightpath.getChannels());
    ...
    public Gv trafficGrooming(LinkedHashMap<String,Nodepair> nodepairlist,LinkedList<Integer>serviceList){
        ...
        for (Link link:layer.getLinklist().values()) {
            sum+=link.getChannels();
            if (maxWave<link.getChannels())
                maxWave=link.getChannels();
        }
        sum=sum/layer.getLinklist().size();
        for (Link l:layer.getLinklist().values()) {
            Variance+=(l.getChannels()-sum)*(l.getChannels()-sum);
        }
        Variance=Variance/layer.getLinklist().size();
        ...
        for (int shuffleNum = 0; shuffleNum < Constant.shuffle; shuffleNum++) {
            ...
            sum1=sum1/layer1.getLinklist().size();
            for (Link l:layer1.getLinklist().values()) {
                Variance1+=(l.getChannels()-sum1)*(l.getChannels()-sum1);
            }
            Variance1=Variance1/layer1.getLinklist().size();
            if (maxWave>maxWave1){
                maxWave=maxWave1;
                gv=newGv;
                layer=layer1;
            }
            else if (maxWave==maxWave1&&Variance>Variance1){
                gv=newGv;
                layer=layer1;
                Variance=Variance1;
            }
...
//                                        routeArr.add(route.getRemCapacity());   //路由的剩余容量
                                        routeArr.add(route.getLength());   //路由的距离
                                    }
...
//                                   System.out.print("途经链路不满足节点对需求！  ");
                                    ...
                                    LinkedList<Integer>routeArr=new LinkedList<>();
                                    for (Route route:routeArrayList) {
                                        routeArr.add(route.getLength());   //路由的跳数
                                    }
                                    Collections.sort(routeArr);  //按从小到大排序
//                                    routeArr.sort(Collections.reverseOrder());  //按从大到小排序
