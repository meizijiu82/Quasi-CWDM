...
//                                    System.out.println("途经链路不满足节点对需求！");
                                    ...
                                    for (Route route:routeArrayList) {
                                        routeArr.add(route.getRemCapacity());   //路由的跳数
                                    }
//                                    Collections.sort(routeArr);  //按从小到大排序
                                    ...
                                    int remCapacity=0;
                                    if (routeArr.size()!=0){
                                        remCapacity=routeArr.getFirst();    //排序后的第一个路由
                                    }
                                    for (Route route:routeArrayList){
...
//                                        if (route.getRemCapacity()==remCapacity&&route.getVlinklist().size()!=0){
...
//                                        }
                        ...
                        Quasi_CWDM7.buildAdaptiveLightpath(layer, gv, BPSK, QPSK, eightQAM, existNodeList, service, nodepair, srcNode, desNode, lightpath, vlink, dijkstra, judge);
                    }
                }
            }
        }
